...
# IP forward to VPN interface
vpn_if="tun0"
eth_if=${server_if}
set_rules "[NAT][POSTROUTING][ACCEPT] vpn forward" "POSTROUTING -t nat -o ${vpn_if} -j MASQUERADE"
set_rules "[FORWARD][ACCEPT] vpn forward" "FORWARD -i ${vpn_if} -o ${eth_if} -m state --state RELATED,ESTABLISHED -j ACCEPT"
set_rules "[FORWARD][ACCEPT] vpn forward" "FORWARD -i ${eth_if} -o ${vpn_if} -j ACCEPT"
